...
#' }
#' @export
cmap_convert<- function(data, cruise, cruise_nickname, project, version = "v1.0") {
  ...
  for(i in 1:length(pop)){
    if(pop[i] == "prochlorococcus" | pop[i] == "bacteria"){
    ...
    long_cell_diameter[[i]] <- paste("50% percentile of", pop[i], "equivalent spherical diameter using", RI)
    ...
    long_carbon_content[[i]] <- paste("50% percentile of", pop[i], "cellular carbon content using", RI)
